...
function getCandidatesFromColumnRefNode(columnRefNode: ColumnRefNode, schema: Schema): CompletionItem[] {
  const tableCandidates = schema.filter(v => v.tableName.includes(columnRefNode.table)).map(v => toCompletionItemFromTable(v))
...
export default function complete(sql: string, pos: Pos, schema: Schema = []) {
  ...
  candidates = candidates.filter(v => v.label.toLowerCase().startsWith(lastToken))
